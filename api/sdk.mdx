...
```typescript
const verification = await clients.compliance.verify({
  type: 'individual',
  firstName: 'John',
  lastName: 'Doe',
  // ...
...
```typescript
const profile = await clients.identity.me();
const aliases = await clients.identity.listAliases();
```
...
```typescript
const organizations = await clients.orgs.list();
const members = await clients.orgs.listMembers('org-id');
```
...
```typescript
const quote = await clients.swap.quote({
  from: 'USD',
  to: 'EUR',
  amount: 100
});
